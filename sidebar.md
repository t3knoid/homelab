---
title: "Sidebar"
---

<!-- 🧱 Platform Foundation -->
<details open>
    <summary>Platform Foundation</summary>
  <details>
    <summary>🖥 Compute & Virtualization</summary>
    <ul>
      <li>[Proxmox](proxmox.md)</li>
      <li>[cloud-init](cloud-init.md)</li>
      <li>[Ansible](ansible.md)</li>
      <li>[Docker](docker.md)</li>
      <li>[WSL2](wsl2.md)</li>
    </ul>
  </details>

  <details>
    <summary>🌐 Networking</summary>
    <ul>
      <li>[Reverse-proxy](reverse-proxy.md)</li>
      <li>[Certbot](certbot.md)</li>
      <li>[Pi-hole DNS](pi-hole_dns.md)</li>
      <li>[DMZ Network Design and Implementation](dmz_network_design_and_implementation.md)</li>
    </ul>
  </details>

  <details>
    <summary>💾 Storage & Data Protection</summary>
    <ul>
      <li>[Ceph](ceph.md)</li>
      <li>[Synology NAS](synology_nas.md)</li>
      <li>[TrueNAS](truenas.md)</li>
      <li>[Proxmox Backup Server](proxmox_backup_server.md)</li>
    </ul>
  </details>
</details>

<!-- 🔐 Identity & Access -->
<details>
  <summary>🔐 Identity & Access</summary>
  <ul>
    <li>[OAuth2 Proxy](oauth2_proxy.md)</li>
    <li>[Domain Controller](domain_controller.md)</li>
    <li>[LDAP](ldap.md)</li>
    <li>[Microsoft Entra ID](microsoft_entra_id.md)</li>
  </ul>
</details>

<!-- 🧩 Core Platform Services -->
<details>
  <summary>🧩 Core Platform Services</summary>
  <ul>
    <li>[PostgreSQL](postgresql.md)</li>
    <li>[Proxmox Backup Server](proxmox_backup_server.md)</li>
    <li>[Autofs](autofs.md)</li>
  </ul>
</details>

<!-- 🧩 Application Workloads -->
<details>
  <summary>🧩 Application Workloads</summary>

  <details>
    <summary>Media Services</summary>
    <ul>
      <li>[Plex](plex.md)</li>
      <li>[Tautulli](tautulli.md)</li>
      <li>[Radarr](radarr.md)</li>
      <li>[Sonarr](sonarr.md)</li>
      <li>[Lidarr](lidarr.md)</li>
      <li>[Sabnzbd](sabnzbd.md)</li>
      <li>[Ombi](ombi.md)</li>
    </ul>
  </details>

  <details>
    <summary>Library & Content</summary>
    <ul>
      <li>[Calibre](calibre.md)</li>
      <li>[Calibre-Web](calibre-web.md)</li>
      <li>[Lazy Librarian](lazy_librarian.md)</li>
    </ul>
  </details>

  <details>
    <summary>Game Servers</summary>
    <ul>
      <li>[Minecraft Server](minecraft_server.md)</li>
    </ul>
  </details>

  <details>
    <summary>Home Automation</summary>
    <ul>
      <li>[Home Assistant](home_assistant.md)</li>
    </ul>
  </details>
</details>

<!-- 📈 Observability & Monitoring -->
<details>
  <summary>📈 Observability & Monitoring</summary>
  <ul>
    <li>[Prometheus](prometheus.md)</li>
    <li>[Grafana](grafana.md)</li>
    <li>[Monitoring & Observability](monitoring_&_observability.md)</li>
  </ul>
</details>

<!-- ⚙️ Automation & Ops -->
<details>
  <summary>⚙️ Automation & Ops</summary>
  <ul>
    <li>[Ansible](ansible.md)</li>
    <li>[Semaphore](semaphore.md)</li>
    <li>[Terraform](terraform.md)</li>
    <li>[Jenkins](jenkins.md)</li>
    <li>[GitHub](github.md)</li>
  </ul>
</details>

<!-- 💻 Infrastructure-as-Code -->
<details>
  <summary>💻 Infrastructure-as-Code</summary>
  <ul>
    <li>[GitHub](github.md)</li>
    <li>[Visual Studio Code](visual_studio_code.md)</li>
    <li>[Code Server](code_server.md)</li>
    <li>[Ansible](ansible.md)</li>
    <li>[Terraform](terraform.md)</li>
  </ul>
</details>

<!-- 📚 Operational Runbooks -->
<details>
  <summary>📚 Operational Runbooks</summary>
  <ul>
    <li>[Runbooks](runbooks.md)</li>
  </ul>
</details>

<!-- 📘 Governance & Documentation -->
<details>
  <summary>📘 Governance & Documentation</summary>
  <ul>
    <li>[Redmine](redmine.md)</li>
    <li>[Home Lab Wiki Mirror to a Static Website Workflow](home_lab_wiki_mirror_to_a_static_website_workflow.md)</li>
  </ul>
</details>
